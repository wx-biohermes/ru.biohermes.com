...
class Content extends \Phpcmf\Model {
    ...
    public function save_content($id, $data, $old = []) {
        ...
        if (defined('IS_MODULE_VERIFY')) {
            // 如果来自审核页面,表示完成审核
            $this->_verify_success($data, $old);
            if (isset($old['verify']['isnew']) && $old['verify']['isnew']) {
                $is_add = 1; // 新增审核成功的
            }
    ...
    public function delete_to_recycle($ids, $note = '无') {
        ...
        foreach ($ids as $id) {
            ...
            if (!$row) {
                ...
                if (!$row) {
                    return dr_return_data(0, dr_lang('内容（%s）不存在', $id));
                }
            }

            if (dr_is_app('cqx') && \Phpcmf\Service::M('content', 'cqx')->is_edit($row['catid'], $row['uid'])) {
    ...
    public function get_admin_list_where($table = '') {
        ...
        return dr_is_app('cqx') ? \Phpcmf\Service::M('content', 'cqx')->get_list_where($table) : '';
    }
