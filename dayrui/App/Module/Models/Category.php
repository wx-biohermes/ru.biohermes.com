...
class Category extends \Phpcmf\Model {
    ...
    // 用于移动时获取的数据
    public function data_for_move($ismain = 0) {

        $cache = [];
        // 全部栏目
        if ($ismain) {
            $data = $this->db->table($this->tablename)->where('ismain=1')->orderBy('displayorder ASC,id ASC')->get()->getResultArray();
        } else {
            $data = $this->db->table($this->tablename)->orderBy('displayorder ASC,id ASC')->get()->getResultArray();
        }
    ...
    // 找到主栏目id 4.6版本改为L方法
    public function get_ismain_id($cat) {
        ...
        if ($cat['pids']) {
            ...
            foreach ($arr as $t) {
                if ($t) {
                    $my = dr_cat_value(\Phpcmf\Service::C()->module['mid'], $t);
                    if ($my['ismain']) {
                        return $t;
                    }
                }
