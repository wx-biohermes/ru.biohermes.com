...
class Auth extends \Phpcmf\Common
{
    ...
    public function add() {
        ...
        $auth = [
            'share_category' => is_array($data['share_category_public']) ? $data['share_category_public'] : [],
            'category' => is_array($data['category_public']) ? $data['category_public'] : [],
        ];
        ...
        \Phpcmf\Service::V()->assign([
            ...
            'menu' => \Phpcmf\Service::M('auth')->_admin_menu(
                [
                    '返回模式选择' => ['member/auth/index', 'fa fa-cog'],
                    dr_lang('%s：用户权限设置', $name) => ['member/auth/add{aid='.$aid.'}', 'fa fa-user'],
                    'help' => [801],
                ]
            ),
            'page' => $page,
